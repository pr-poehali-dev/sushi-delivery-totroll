...
const MobileDeliveryToggle: React.FC<MobileDeliveryToggleProps> = ({
  ...
  return (
    <div className="md:hidden bg-[#FFE4B5]/50 py-2 border-t border-b border-[#FFE4B5] mb-4">
      <div className="container mx-auto px-4">
        <div className="flex items-center justify-center">
          <span className={`text-sm mr-3 ${!isDelivery ? 'font-bold text-black' : 'text-black/70'}`}>
            Самовывоз
          </span>
          <DeliveryToggle isDelivery={isDelivery} setIsDelivery={setIsDelivery} />
          <span className={`text-sm ml-3 ${isDelivery ? 'font-bold text-black' : 'text-black/70'}`}>
            Доставка
          </span>
        </div>
      </div>
